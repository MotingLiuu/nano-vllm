...
class ModelRunner:
    ...
    # Summary:
    # 1. if is_prefill or self.enforce_eager or input_ids.size(0) > 512, run the model with self.model(input_ids, position)
    # 2. else, run the model with self.model.compute_logits(self.model(input_ids, position))
